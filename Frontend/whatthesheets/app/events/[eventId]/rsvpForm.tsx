...
export function RsvpForm({ eventData }: RsvpFormProps) {
  const formSchema = Yup.object().shape({
    ...
    plusOneName: Yup.string().when(
      "addPlusOne",
      (addPlusOneValue: any, schema: any) => {
        return addPlusOneValue
          ? schema.required("Please enter the name of your plus-one.")
          : schema.notRequired();
      }
    ),
  });
  ...
  return (
    <div className="mx-auto font-sans text-center justify-between gap-x-6 p-6 lg:px-8">
      <div className="mx-10 mb-8 border-none shadow-sm p-6">
        <h2 className="p-6 my-3 text-yellow-500 text-4xl font-bold">
          Make RSVP
        </h2>
        <form
          onSubmit={formik.handleSubmit}
          className="mx-auto mt-6 max-w-lg grid grid-cols-2 gap-4"
        >
          <div>
            <label
              htmlFor="fristName"
              className="block text-sm font-semibold font-sans leading-6  text-yellow-500"
            >
              {" "}
              First Name
            </label>
            <input
              ...
              className={`block w-full rounded-md border-0 px-3.5 py-2 text-slate-900 shadow-sm ring-1 ring-inset ${
                formik.touched.firstName && formik.errors.firstName
                  ? "ring-red-500"
                  : "ring-slate-300"
              } focus:ring-2 focus:ring-inset focus:ring-amber-400 sm:text-sm sm:leading-6`}
            />
            {formik.touched.firstName && formik.errors.firstName && (
          ...
          <div>
            <label
              htmlFor="lastName"
              className="block text-sm font-sans leading-6  text-yellow-500"
            >
              Last Name
            </label>
            <input
              ...
              onChange={formik.handleChange}
              onBlur={formik.handleBlur}
              className={`block w-full rounded-md border-0 font-sanspx-3.5 py-2 text-slate-900 shadow-sm ring-1 ring-inset ${
          ...
          <button
            ...
            type="submit"
            className="block w-full font-sans rounded-md bg-yellow-500 px-3.5 py-2 text-center text-sm font-semibold text-white shadow-sm hover:bg-slate-500"
          >
            Add
          </button>
        </form>
                ...
                <div className="mt-2">
                  {rsvpId ? (
                    <p className="text-sm text-gray-500">
                      ...
                      Please have this ID if you need to contact Host to edit
                      the reservation.
                    </p>
                  ) : (
                    <p className="text-sm text-gray-500">
                      Failed to submit RSVP: {submitError}. Please contact Site
                      Administrator.
                    </p>
                  )}
                </div>
